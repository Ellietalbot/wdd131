...
function renderTasks(tasks) {
  ...
  // loop through the tasks array. transform (map) each task object into the appropriate HTML to represent a to-do.
    ...
    todolist.innerHTML = ''
    const todolist_tasks = tasks.map(`
    ...
    todolist.innerHTML = todolist_tasks
...
function newTask() {
  ...
  // render out the list
  const task_input = document.getElementById('todo').value;
  const new_task = {detail:task_input, completed:false};
  tasks.push(new_task)
  renderTasks()
}
...
document.querySelector('#submitTask').addEventListener('submit', newTask)